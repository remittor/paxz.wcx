...
#define PK_CAPS_NEW        0x0001  /* Can create new archives              */
#define PK_CAPS_MODIFY     0x0002  /* Can modify exisiting archives        */
#define PK_CAPS_MULTIPLE   0x0004  /* Archive can contain multiple files   */
#define PK_CAPS_DELETE     0x0008  /* Can delete files                     */
#define PK_CAPS_OPTIONS    0x0010  /* Has options dialog                   */
#define PK_CAPS_MEMPACK    0x0020  /* Supports packing in memory           */
#define PK_CAPS_BY_CONTENT 0x0040  /* Detect archive type by content       */
#define PK_CAPS_SEARCHTEXT 0x0080  /* Allow searching for text in archives created with this plugin */
#define PK_CAPS_HIDE       0x0100  /* Show as normal files (hide packer icon), open with Ctrl+PgDn, not Enter */
#define PK_CAPS_ENCRYPT    0x0200  /* Plugin supports PK_PACK_ENCRYPT option */
...
typedef struct {
    UINT16   seconds : 5;   /* seconds divided in half */
    UINT16   minutes : 6;
    UINT16   hours   : 5;
    UINT16   days    : 5;
    UINT16   months  : 4;
...
/* callback functions called by the DLL Ask to swap disk for multi-volume archive */
typedef int (WINAPI * tChangeVolProc) (char * ArcName, int Mode);
typedef int (WINAPI * tChangeVolProcW) (LPCWSTR ArcName, int Mode);

/* Notify that data is processed - used for progress dialog */
typedef int (WINAPI * tProcessDataProc) (char * FileName, int Size);
typedef int (WINAPI * tProcessDataProcW) (LPCWSTR FileName, int Size);

/* for cipher */
typedef int (WINAPI * tPkCryptProc) (int CryptoNr, int Mode, char * ArchiveName, char * Password, int maxlen);
typedef int (WINAPI * tPkCryptProcW) (int CryptoNr, int Mode, LPCWSTR ArchiveName, LPCWSTR Password, int maxlen);
